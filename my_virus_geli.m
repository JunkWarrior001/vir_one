...
while t<=50
  ...
  [xiedai,index_xiedaipos,index_xiedai,ganran,index_ganran,index_ganranpos,jiankang,index_jiankang,index_jiankangpos,tim]=panduanganran(xiedai,r,index_xiedaipos,index_xiedai,ganran,index_ganran,index_ganranpos,jiankang,index_jiankang,index_jiankangpos,tim,p3,ryuanshi);
...
function [xiedai,index_xiedaipos,index_xiedai,ganran,index_ganran,index_ganranpos,jiankang,index_jiankang,index_jiankangpos,tim]=panduanganran(xiedai,r,index_xiedaipos,index_xiedai,ganran,index_ganran,index_ganranpos,jiankang,index_jiankang,index_jiankangpos,tim,p3,ryuanshi)
    for i=1:200
        judge=0;
        if tim(i)>=20
            if index_ganran(i)==1       %将感染者移出（隔离），并在合法区域内随机生成一个健康者保证总人数不变
                index_ganran(i)=0;
                index_ganranpos(r(i,1),r(i,2))=0;
                index_jiankang(i)=1;
                rr=randi([1,200]);
                while (index_jiankangpos(ryuanshi(rr,1),ryuanshi(rr,2))==1)||(index_xiedaipos(ryuanshi(rr,1),ryuanshi(rr,2))==1)||(index_ganranpos(ryuanshi(rr,1),ryuanshi(rr,2))==1)
                    rr=randi([1,200]);
                end
                index_jiankangpos(ryuanshi(rr,1),ryuanshi(rr,2))=i;
                continue;
            end
